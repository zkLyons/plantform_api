...
// 在一个名为 docMeta 的静态数据结构中，根据给定的 category (类别) 和 slug (唯一标识)，Hook 向外部（调用它的组件）返回一个包含当前文档内容、加载状态和错误信息的对象。
export function useDoc(category, slug) {
    const [content, setContent] = useState('')
    const [loading, setLoading] = useState(true)
    const [error, setError] = useState(null)

    // 这是 Hook 的核心。它会在组件首次加载时，以及每当 category 或 slug 发生变化时执行。
    useEffect(() => {
        setLoading(true)
        setError(null)
        // 将 category 和 slug（例如 'tutorials', 'hello-world'）转换成一个标准的文件路径字符串（例如 '../data/docs/tutorials/hello-world.md'）。
        const path = resolveDocPath(category, slug)
        const loader = allDocs[path]
        // loader:() => import("/src/data/docs/api/quickstart/digital-twin-api.md?import&raw").then(m => m["default"])

        if (loader) {
            loader()
                .then(raw => {
                    // 使用正则表达式益处markdown文件头部的‘frontmatter’元数据
    ...
    // Hook 向外部（调用它的组件）返回一个包含当前文档内容、加载状态和错误信息的对象。
    return { content, loading, error }
}

// 在一个名为 docMeta 的静态数据结构中，根据给定的 category (类别) 和 slug (唯一标识)，查找并返回对应文档的元数据对象。
