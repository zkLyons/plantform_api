...
const METHOD_CN = {
    ...
    setFogParam: '雾效',
    setCloudParam: '云效',
    setCloudThickness: '云层厚度',
    setCloudDensity: '云层密度',
    setDateTime: '日期时间'
}
/**
 * 
 * 作用：它接收一个值 v 和一个范围 [lo, hi]（lo 是下限，hi 是上限），并确保返回的值不会超出这个范围。
如果 v 小于 lo，它返回 lo。
如果 v 大于 hi，它返回 hi。
如果 v 在 lo 和 hi 之间，它返回 v 本身。} v 
 */
const clamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v))
/**
它接收一个数字 n，并将其转换为一个至少两位的十六进制字符串。这在构建 CSS 颜色代码（如 #RRGGBB）时非常有用，因为每个颜色分量都必须是两位。
实现步骤：
Math.round(n): 先将数字四舍五入为整数。
.toString(16): 将该整数转换为十六进制字符串（例如，10 变成 'a'，255 变成 'ff'）。
.padStart(2, '0'): 如果转换后的字符串不足两位，就在前面补一个 '0'。例如，'a' 会变成 '0a'。 
 */
const to2 = n => Math.round(n).toString(16).padStart(2, '0')
// 以上，这两个函数是协同工作的，主要目的是将一个表示颜色的数组（如 [255, 0, 0]）转换成一个标准的十六进制颜色字符串
...
// hexToArr('#rrggbb', scale255, alpha) -> [r,g,b,(a)](0..1 或 0..255)
...
/**
 * ns,         // 命名空间，如 "camera" 或 null
      method,     // 方法名，如 "set"
      open,       // 左括号的起始索引
      close,      // 右括号的结束索引
      argsStart: open + 1, // 参数文本的起始索引
      argsText: code.slice(open + 1, close) // 括号内的原始参数文本
 * 
 */
// 它的核心作用是在编辑器的一段代码中，找出第一个可以被参数面板解析和控制的 API 调用,例如知道fdapi.camera.set(...) 这种调用，并提取出方法名、命名空间、参数文本等信息，以便后续生成参数控件和处理用户交互。
...
// 这个函数和上面的函数类似，只不过它会找到所有符合条件的 API 调用，并将它们全部收集到一个数组中返回。
...
// 它的核心作用是在一段给定的代码字符串中，从一个指定的左括号 ( 开始，智能地寻找到与之配对的那个右括号 ) 的位置。
...
// 它的核心作用是将一个包含函数参数的字符串，按照最外层的逗号进行分割，同时忽略掉嵌套结构（如数组 []、对象 {}、括号 ()）和字符串内部的逗号。
// 一个简单的 string.split(',') 无法胜任，因为它会错误地分割复杂参数，例如： fdapi.someMethod([10, 20], { a: 1, b: 2 }, "hello, world")---》splitTopLevel 就能正确地将上面的参数分割成三个部分：[10, 20]、{ a: 1, b: 2 } 和 "hello, world"
...
function parseObject(objText, objBase) {
    ...
    while (i < n) {
        ...
        if (body[i] === '"' || body[i] === "'") {
            const q = body[i++]
            let s = ''
            while (i < n && body[i] !== q) s += body[i++]
            i++
            key = s
        } else {
            let s = ''
            while (i < n && /[\w$]/.test(body[i])) s += body[i++]
            key = s
        }
        skipWs()
        if (body[i] !== ':') {
            // 非 key:value（shorthand / spread），跳到下个顶层逗号
            let depth = 0,
                str = null
            while (i < n) {
                const c = body[i]
...
function serialize(ctrl, value) {
    switch (ctrl.kind) {
        ...
        case 'slider':
            return String(value)
        case 'enum':
            return typeof ctrl.value === 'string' ? (ctrl.quote || "'") + value + (ctrl.quote || "'") : String(value)
        case 'string': {
            const q = ctrl.quote || "'"
            return q + value + q
        }
        ...
        case 'color': {
            ...
            const q = ctrl.quote || "'"
            return q + value + q // hexstr / name 统一回写为 '#rrggbb'
        }
        default:
            return value
    }
        ...
        try {
            ...
            for (const call of calls) {
                ...
                if (objStart >= 0 && objStart < 3) {
                    ...
                    const params = (PARAM_META.byNs[call.ns] && PARAM_META.byNs[call.ns].methods[call.method] && PARAM_META.byNs[call.ns].methods[call.method].params) || []
                            ...
                            <React.Fragment key={g.title + gi}>
                                ...
                                {g.controls.map((c, i) => (
                                    <div className="lilgui-row" key={c.key + '-' + gi + '-' + i} title={c.tip || c.key}>
                                        ...
                                        <span className="lilgui-ctl">
                                            <Control ctrl={c} onChange={v => applyChange(c, v)} />
                                        </span>
                                    </div>
                                ))}
                            </React.Fragment>
    ...
    switch (ctrl.kind) {
        ...
        case 'slider':
            return (
                <>
                    ...
                    <input
                        type="number"
                        className="lilgui-num lilgui-numbox"
                        step={ctrl.step}
                        value={local}
                        onChange={e => {
                            const v = Number(e.target.value)
        ...
        case 'number':
            return (
                <input
                    type="number"
                    className="lilgui-num"
                    value={local}
                    onChange={e => {
                        const v = e.target.value === '' ? '' : Number(e.target.value)
                        setLocal(v)
                        if (e.target.value !== '') onChange(v)
                    }}
                />
            )
        case 'color':
            return (
                <>
                    ...
                    <input
                        type="text"
                        className="lilgui-txt lilgui-hex"
                        value={local}
                        onChange={e => {
                            setLocal(e.target.value)
