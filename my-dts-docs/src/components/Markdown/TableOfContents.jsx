...
export default function TableOfContents() {
  ...
  return (
    <nav className="hidden xl:block w-56 shrink-0" aria-label="目录导航">
      <div className="sticky top-24 pl-6 border-l border-edge">
          ...
          {headings.map((h) => (
            <li key={h.id}>
              <button
                ...
                className={`block w-full text-left text-[12px] leading-snug transition-smooth ${
                  h.level === 3 ? 'pl-3' : ''
                } ${
                  activeId === h.id
                    ? 'text-accent font-medium'
                    : 'text-muted hover:text-[#2d2926]'
                }`}
              >
                {h.text}
              </button>
