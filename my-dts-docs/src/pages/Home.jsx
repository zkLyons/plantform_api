        ...
        <div className="min-h-screen">
            ...
            <section className="relative overflow-hidden">
                ...
                <div className="relative max-w-4xl mx-auto px-6 pt-28 pb-24 text-center">
                    ...
                    <h1 className="text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight mb-6 leading-[1.1]">
                        ...
                        <br />
                        <span className="text-accent">数字孪生 API 文档</span>
                    </h1>

                    <p className="text-base sm:text-lg text-muted max-w-xl mx-auto mb-10 leading-relaxed">飞渡科技云渲染数字孪生平台 JavaScript SDK 完整参考。 覆盖相机、图层、仿真、分析等 92 个 API 类，500+ 方法。</p>

                    <div className="flex flex-wrap justify-center gap-3">
                        <Link to="/docs/tutorials/hello-world" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-accent text-white font-medium text-sm hover:bg-accent-hover transition-smooth">
                            快速开始
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <path d="M5 12h14M12 5l7 7-7 7" />
                            </svg>
                        </Link>
                        <Link to="/docs/tutorials/architecture" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-edge text-muted font-medium text-sm bg-elevated hover:border-edge-hover hover:text-[#2d2926] transition-smooth">
                            架构概览
                        </Link>
                    </div>
                </div>
            ...
            <section className="max-w-4xl mx-auto px-6 py-20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {features.map(f => (
                        <Link key={f.title} to={f.to} className="group relative p-5 rounded-xl border border-edge bg-elevated hover:border-edge-hover hover:shadow-sm transition-smooth">
                            <div className="w-10 h-10 rounded-lg bg-accent/8 text-accent flex items-center justify-center mb-4 group-hover:bg-accent/12 transition-smooth">{f.icon}</div>
            ...
            <section className="max-w-4xl mx-auto px-6 pb-24">
                ...
                <div className="flex flex-wrap gap-2">
                    {quickLinks.map(link => (
                        <Link key={link.to} to={link.to} className="px-4 py-2 rounded-lg border border-edge text-sm text-muted bg-elevated hover:text-accent hover:border-accent/30 transition-smooth">
                            {link.label}
                        </Link>
                    ))}
                </div>
