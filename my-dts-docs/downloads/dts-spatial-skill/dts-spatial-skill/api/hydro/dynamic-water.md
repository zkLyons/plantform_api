...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `add` | 添加一个或多个DynamicWater对象 | 向场景批量添加对象 |
| `clear` | 删除场景中所有的DynamicWater | 清空全部对象，重置图层 |
| `delete` | 删除一个或多个DynamicWater对象 | 按 ID 移除指定对象 |
| `focus` | 自动定位到合适的观察距离 | 相机定位到目标，点击联动跳转 |
| `focusAll` | 自动定位到能观察所有DynamicWater对象的合适距离 | 相机定位到全部对象的合适视角 |
| `get` | 根据ID获取DynamicWater的详细信息 | 查询对象信息，用于业务联动 |
| `setCoordinates` | 设置DynamicWater的坐标信息 |  |
| `setStyle` | 设置动态水的样式 |  |
| `update` | 修改一个或多个DynamicWater对象 | 运行时动态更新对象属性/状态 |
