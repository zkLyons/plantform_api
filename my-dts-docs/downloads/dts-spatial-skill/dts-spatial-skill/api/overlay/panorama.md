...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `add` | 添加一个或多个Panorama全景图对象 | 向场景批量添加对象 |
| `clear` | 清空场景中所有的Panorama | 清空全部对象，重置图层 |
| `delete` | 删除一个或多个Panorama对象 | 按 ID 移除指定对象 |
| `enter` | 进入全景图模式 |  |
| `exit` | 退出全景图模式 |  |
| `focus` | 自动定位到合适的观察距离 | 相机定位到目标，点击联动跳转 |
| `get` | 根据ID获取Panorama的详细信息 | 查询对象信息，用于业务联动 |
| `switchMode` | 切换显示模式，即全景图模式和三维场景模式切换 |  |
| `update` | 修改一个或多个Panorama对象 | 运行时动态更新对象属性/状态 |
