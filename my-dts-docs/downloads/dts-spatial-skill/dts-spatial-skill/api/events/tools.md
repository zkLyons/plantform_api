...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `exportSkyline` | 导出天际线分析图片， |  |
| `getUIPanel` | 查询系统UI功能操作面板的显示状态 |  |
| `hideUIPanel` | 隐藏系统UI功能操作面板 |  |
| `lineIntersect` | 单条线段求交，如果线段上有多个对象则返回距离折线起点位置最近的对象信息，如果折线上没有对… |  |
| `linesIntersect` | 多条线段批量求交 |  |
| `replaceTextureByImage` | 使用图片替换纹理 |  |
| `replaceTextureByUrl` | 使用网页替换纹理 |  |
| `replaceTextureByVideo` | 使用视频流替换纹理 |  |
| `restoreTexture` | 恢复纹理 |  |
| `riverCrossSection` | 根据河道的坐标点和tif高程对河道横断面进行分析 |  |
| `riverCrossSectionByShp` | 根据河道的shp文件和tif高程对河道横断面进行分析 |  |
| `setMeasurement` | 设置测量模式及相关参数 |  |
| `showClipPanel` | 打开剖切功能操作面板 |  |
| `showMeasurePanel` | 打开测量功能操作面板 |  |
| `showUIPanel` | 显示系统UI功能操作面板 |  |
| `startContourLineAnalysis` | 开始等高线分析，适用于地形 |  |
| `startCutFillAnalysis` | 开始填挖方分析，适用于地形 |  |
| `startFloodFill` | 开始水淹分析 |  |
| `startGeometryEdit` | 开始顶点编辑，目前支持watermesh polygon 3dpolygon polyl… |  |
| `startMeasurement` | 开始测量，用户可以在三维场景中点击鼠标进行测量 |  |
| `startPlaneClip` | 开始面剖切 |  |
| `startPolygonClip` | TileLayer多边形剖切， |  |
| `startSkylineAnalysis` | 开始天际线分析 |  |
| `startSunshineAnalysis` | 开始日照分析 |  |
| `startTerrainSlopeAnalysis` | 开始坡度坡向分析，适用于地形3dt数据 |  |
| `startViewDomeAnalysis` | 开始开敞度分析 |  |
| `startViewshedAnalysis` | 开始可视域分析 |  |
| `startVisiblityAnalysis` | 开始通视分析 |  |
| `startVolumeClip` | 开始体剖切 |  |
| `stopClip` | :::caution 已废弃 |  |
| `stopContourLineAnalysis` | 停止等高线分析 |  |
| `stopCutFillAnalysis` | 停止填挖方分析 |  |
| `stopFloodFill` | 停止水淹分析 |  |
| `stopGeometryEdit` | 退出顶点编辑 |  |
| `stopMeasurement` | 停止测量 |  |
| `stopPlaneClip` | 停止面剖切 |  |
| `stopPolygonClip` | 停止多边形剖切 |  |
| `stopSkylineAnalysis` | 结束天际线分析 |  |
| `stopSunshineAnalysis` | 停止日照分析 |  |
| `stopTerrainSlopeAnalysis` | 停止坡度坡向分析 |  |
| `stopViewDomeAnalysis` | 停止开敞度分析 |  |
| `stopViewshedAnalysis` | 停止可视域分析 |  |
| `stopVisiblityAnalysis` | 停止通视分析 |  |
| `stopVolumeClip` | 停止体剖切 |  |
| `updateVolumeClip` | 更新体剖切 |  |
