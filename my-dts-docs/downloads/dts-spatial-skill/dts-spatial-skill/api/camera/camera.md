...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `cancelFollow` | 取消相机自动跟随，支持取消CustomObject、Vehicle、Vehicle2、T… |  |
| `enterWorld` | 进入世界（三维场景），即从地球飞入到三维场景 |  |
| `exitWorld` | 退出世界（三维场景），即从三维场景飞入到地球 |  |
| `exportOrthoImage` | 根据传入的参数导出相机位置对应的正交投影图片 |  |
| `flyAround` | 相机环绕指定位置旋转一周 |  |
| `get` | 获取当前的相机位置 | 查询对象信息，用于业务联动 |
| `getAnimationImage` | 根据导览名称获取对应导览缩略图的base64字符串，默认图片格式png，尺寸：128*128 |  |
| `getAnimationList` | 获取当前acp里所有导览的信息 |  |
| `getEulerAngle` | 根据空间两点计算欧拉角 |  |
| `lockByBBox` | 锁定相机的交互范围，仅允许在BBox内交互 |  |
| `lookAt` | 通过观察点设置相机位置 |  |
| `lookAtBBox` | 通过BBox设置相机 |  |
| `moveBackward` | 后退 |  |
| `moveDown` | 下降 |  |
| `moveForward` | 前进 |  |
| `moveLeft` | 左移 |  |
| `moveRight` | 右移 |  |
| `moveUp` | 上升 |  |
| `pauseAnimation` | 暂停播放动画导航 |  |
| `playAnimation` | 按传入索引序号的顺序播放一个或多个动画导览 |  |
| `resumeAnimation` | 恢复播放动画导航 |  |
| `set` | 设置相机位置，这是最常见的参数形式，另外两种参数形式， |  |
| `stop` | 停止 | 停止播放 |
| `stopAnimation` | 停止播放动画导航 |  |
| `turnDown` | 低头 |  |
| `turnLeft` | 左转 |  |
| `turnRight` | 右转 |  |
| `turnUp` | 抬头 |  |
| `unlock` | 解锁相机的交互范围，恢复自由交互 |  |
| `useOldDataFormat` | 设置是否使用旧版本的数据格式（2021.03.17之前的版本），这是一个全局的设置。 |  |
