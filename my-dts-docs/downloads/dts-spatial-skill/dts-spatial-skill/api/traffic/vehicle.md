...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `add` | 添加一个或多个Vehicle对象 | 向场景批量添加对象 |
| `callBatchFunction` | 调用多个Vehicle对象的多个蓝图函数 |  |
| `clear` | 清空场景中所有的Vehicle对象 | 清空全部对象，重置图层 |
| `delete` | 删除一个或多个Vehicle对象 | 按 ID 移除指定对象 |
| `focus` | 自动定位到合适的观察距离 | 相机定位到目标，点击联动跳转 |
| `get` | 根据ID获取Vehicle对象的详细信息 | 查询对象信息，用于业务联动 |
| `hide` | 隐藏一个或多个Vehicle对象 | 按业务条件隐藏对象 |
| `moveTo` | 设置Vehicle对象行驶（根据实时获取的GPS数据运动） | 驱动对象移动到目标位置 |
| `pause` | 暂停指定的载具运动 | 暂停播放 |
| `resume` | 恢复指定的载具运动 | 恢复播放 |
| `setWayPoint` | 设置Vehicle对象行驶的路径点（已知路径点 轨迹动画） |  |
| `show` | 显示一个或多个Vehicle对象 | 按业务条件显示对象 |
| `start` | 启动指定的载具在某个时刻开始运动 | 启动该功能/交互 |
| `stop` | 停止指定的载具运动 | 停止播放 |
| `update` | 修改一个或多个Vehicle对象 | 运行时动态更新对象属性/状态 |
