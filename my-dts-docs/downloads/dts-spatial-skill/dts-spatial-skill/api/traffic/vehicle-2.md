...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `add` | 添加一个或多个Vehicle2对象 | 向场景批量添加对象 |
| `clear` | 清空场景中所有的Vehicle2对象 | 清空全部对象，重置图层 |
| `delete` | 删除一个或多个Vehicle2对象 | 按 ID 移除指定对象 |
| `focus` | 自动定位载具到合适的观察距离 | 相机定位到目标，点击联动跳转 |
| `get` | 根据ID查询Vehicle2对象的详细信息 | 查询对象信息，用于业务联动 |
| `hide` | 隐藏一个或多个Vehicle2对象 | 按业务条件隐藏对象 |
| `moveTo` | 设置Vehicle2对象行驶到具体位置（根据实时获取的GPS数据运动） | 驱动对象移动到目标位置 |
| `setFollow` | 设置载具自动跟随相机 |  |
| `setViewportVisible` | 多视口状态下，设置Vehicle2对象在各视口的可见性 |  |
| `show` | 显示一个或多个Vehicle2对象 | 按业务条件显示对象 |
| `update` | 修改一个或多个Vehicle2对象 | 运行时动态更新对象属性/状态 |
