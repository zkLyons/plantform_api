...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `add` | 添加一个或多个OceanHeatMap海洋热力图对象，数据源为.tif文件。 | 向场景批量添加对象 |
| `clear` | 删除场景中所有的OceanHeatMap | 清空全部对象，重置图层 |
| `delete` | 删除一个或多个OceanHeatMap对象 | 按 ID 移除指定对象 |
| `focus` | 自动定位到合适的观察距离 | 相机定位到目标，点击联动跳转 |
| `get` | 根据ID获取OceanHeatMap的详细信息 | 查询对象信息，用于业务联动 |
| `hide` | 隐藏OceanHeatMap | 按业务条件隐藏对象 |
| `show` | 显示OceanHeatMap | 按业务条件显示对象 |
| `update` | 修改一个或多个OceanHeatMap对象 | 运行时动态更新对象属性/状态 |
