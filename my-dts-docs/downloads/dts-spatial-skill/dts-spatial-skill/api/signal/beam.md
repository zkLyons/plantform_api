...
| 方法 | 说明 | 适用业务场景 |
|------|------|------------|
| `add` | 添加一个或多个Beam对象 | 向场景批量添加对象 |
| `clear` | 删除场景中所有的Beam | 清空全部对象，重置图层 |
| `delete` | 删除一个或多个Beam对象 | 按 ID 移除指定对象 |
| `focus` | 自动定位到合适的观察距离 | 相机定位到目标，点击联动跳转 |
| `get` | 根据ID获取Beam的详细信息 | 查询对象信息，用于业务联动 |
| `hide` | 隐藏Beam | 按业务条件隐藏对象 |
| `hideAll` | 隐藏所有Beam | 一键隐藏全部对象 |
| `setColor` | 设置Color |  |
| `setCoordinates` | 设置Coordinates |  |
| `setDuration` | 设置Duration |  |
| `setInterval` | 设置Interval |  |
| `setThickness` | 设置Thickness |  |
| `setVelocity` | 设置Velocity |  |
| `show` | 显示Beam | 按业务条件显示对象 |
| `showAll` | 显示所有Beam | 一键显示全部对象 |
| `update` | 修改一个或多个Beam对象 | 运行时动态更新对象属性/状态 |
