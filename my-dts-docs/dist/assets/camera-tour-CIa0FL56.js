...
description: "CameraTour 是相机导览（漫游）动画对象，用于按关键帧序列驱动相机沿预设路线自动飞行、播放、暂停与停止，实现自动巡游展示。"\r
---\r
\r
# CameraTour\r
\r
CameraTour 是相机导览（漫游）动画对象，用于按关键帧序列驱动相机沿预设路线自动飞行、播放、暂停与停止，实现自动巡游展示。\r
\r
...
\r
- **功能介绍**：CameraTour 是相机导览（漫游）动画对象，用于按关键帧序列驱动相机沿预设路线自动飞行、播放、暂停与停止，实现自动巡游展示。\r
- **别名 / 不同行业叫法**：漫游 / 导览 / 巡游 / 飞行路线 / 关键帧动画 / 自动巡航；汇报中也称“一键漫游”“自动飞行”。\r
- **适用行业**：展厅汇报、智慧城市、智慧园区、智慧交通、能源、应急指挥。\r
- **使用场景**：\r
  - 展厅或领导汇报中自动巡游城市/园区，串联多个重点区域形成讲解动线。\r
  - 项目开场或大屏轮播时循环播放预设飞行路线，无人值守自动展示。\r
  - 应急复盘按时间轴沿事件发生路径漫游回放现场态势。\r
- **注意事项**：\r
  - 路线由 CameraTourData 与 CameraTourKeyFrame 组成，需先 add 再 play。\r
  - 关键帧坐标系须与工程一致；关键帧不宜过密以保证飞行平滑。\r
  - 播放期间应避免与手动相机操作或跟随同时进行，防止视角抖动。\r
\r
\r
## 方法列表\r
\r
